		...
		public class Bone: Point {
			...
			public void SolverSwing(Bone[] bones, int index, Vector3 swingTarget, float weight = 1f) {
				...
				if (weight >= 1f) {
					...
					//IKSolver.SolverRotateBonesAroundPoint(bones, index, solverPosition, r);
					return;
				}
				
				//IKSolver.SolverRotateBonesAroundPoint(bones, index, solverPosition, Quaternion.Lerp(Quaternion.identity, r, weight));
				for (int i = index; i < bones.Length; i++) {
